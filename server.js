...
// server.js 파일 또는 해당 기능을 처리하는 파일에 추가해주세요.

app.post("/logout", (req, res) => {
  req.logout(function (err) {
    if (err) {
      console.error(err);
      return res.sendStatus(500); // 오류 상태 코드를 반환합니다.
    }
    return res.sendStatus(200); // 성공 상태 코드를 반환합니다.
  });
});
...
app.get("/user_page", logincheck, function (req, res) {
  ...
  db.collection("DB_bookUpload")
    .find({ db_uploader: req.user.db_id })
    .toArray(function (err, result) {
      // console.log(result);
      // console.log(req.user);
      ...
      try {
        ...
        // 없는 정보. 회원 신규 생성.
          ...
          const regi = await new Promise((resolve, reject) => {
            db.collection("DB_user").insertOne(
              {
                db_id: get_id,
                db_pw: get_pw,
                db_name: get_nickname,
                admin: false,
              },
              function (err, res) {
                if (err) reject(err);
  ...
  db.collection("DB_user").findOne(
    ...
    function (err, result) {
      ...
      if (result) {
        ...
        res.write(
          "<script>alert('ID is already registered. Please check again.')</script>"
        );
        res.write('<script>window.location=" /login_sign "</script>');
      } else {
        db.collection("DB_user").insertOne(
          {
            db_id: req.body.regi_id,
            db_pw: req.body.regi_pw,
            db_name: req.body.regi_name,
            admin: false,
          },
          function (err, res) {
            if (err) throw err;
...
app.post("/used_write", logincheck, upload.single("img"), function (req, res) {
  db.collection("DB_postCount").findOne(
    { name: "DB_PSCNT" },
    function (err, res1) {
...
app.get("/used_view/:Viewid", (req, res) => {
  db.collection("DB_bookUpload").findOne(
    { db_upload_post: parseInt(req.params.Viewid) },
    function (err, result) {
      // console.log(result);
...
app.get("/go_used_store", async (req, res) => {
  ...
  try {
    const crawledData =
      await require("./routes/usedstore_crawling.js").crawlData();
...
app.put("/edit", logincheck, upload.single("img"), function (req, res) {
  ...
  // DB 전송
  db.collection("DB_bookUpload").updateOne(
    { db_upload_post: parseInt(number) },
    {
      $set: {
...
app.post("/chatList", logincheck, function (req, res) {
  ...
  db.collection("DB_chatList").findOne(
    ...
    function (err, result) {
      ...
      if (
        result ||
        CHATINFO.member[0].toString() === CHATINFO.member[1].toString()
      ) {
        // 이미 해당 채팅방이 존재하는 경우
        const response = {
          message: message,
        ...
        // 해당 채팅방이 존재하지 않는 경우
        db.collection("DB_chatList").insertOne(
          CHATINFO,
          function (err, result) {
            if (err) {
              // 오류 처리
...
app.post("/message", logincheck, function (req, res) {
  ...
  db.collection("DB_message")
    .insertOne(generateChat)
    .then((result) => {
      res.send(result);
    });
});
